...
surface_set_target(surf);
	...
	gpu_set_colorwriteenable(0, 0, 0, 1);
		gpu_set_blendmode(bm_subtract);
			shader_set(showUnderValue);
					var fullAlphaBelowValue =  shader_get_uniform(showUnderValue, "fullAlphaBelowValue");
					shader_set_uniform_f(fullAlphaBelowValue, alpha);
	
					var sprW = 64;
	
					for (var i = 0; i < display_get_width(); i+=sprW) {
						var ysc = string_height(orgStr) / sprW + 0.6;
						draw_sprite_ext(sAreaReveal, 0, i, stringY-12, 1, ysc, 0, c_white, 1);
					}
			shader_reset();
