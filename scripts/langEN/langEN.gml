//@desc EN Language File Contents.
//
//
//
function langEN() {
	lang[? "c.cutscene_name.example"]                   = [["generic text", "c", "s", "v", ]];

	lang[? "dialogue.randomnpc.greetings"]              = [["Hey There!", "0", ], ["Greetings!", "0", ]];
	lang[? "dialogue.randomnpc.adventurer0"]            = [["Have you seen a {beastiary.magpie} before?", "0", ], ["People brought them over from the New Continent as pets, but now, they\'re a problem from the local wildlife.", "0", ]];

	lang[? "dialogue.shopkeeper.main"]                  = [["Hey There!", "1", ]];

	lang[? "dialogue.answer.shop"]                      = "Shop";
	lang[? "dialogue.answer.talk"]                      = "Talk";
	lang[? "dialogue.answer.leave"]                     = "Leave";
	lang[? "dialogue.answer.silly"]                     = "Huh?";

	lang[? "ui.interact.interact"]                      = "Interact";
	lang[? "ui.interact.listen"]                        = "Listen";
	lang[? "ui.interact.silly"]                         = "Shoot into Sky";
	lang[? "ui.interact.rest"]                          = "Rest";
	lang[? "ui.interact.talk"]                          = "Talk";
	lang[? "ui.interact.inspect"]                       = "Inspect";

	lang[? "ui.instruction.back"]                       = "Back";
	lang[? "ui.instruction.select"]                     = "Select";
	lang[? "ui.instruction.forge"]                      = "Forge Menu";

	lang[? "ui.tutorial.walljump"]                      = "Press Jump when on a wall to preform a Wall Jump";

	lang[? "menu.page.main"]                            = "Game Paused";
	lang[? "menu.page.options"]                         = "Options";
	lang[? "menu.page.gamesettings"]                    = "Game Settings";
	lang[? "menu.page.feedback"]                        = "Feedback";
	lang[? "menu.page.audio"]                           = "Audio";
	lang[? "menu.page.back"]                            = "Back";
	lang[? "menu.page.debugsettings"]                   = "Debug Settings";
	lang[? "menu.page.statistics"]                      = "Statistics";

	lang[? "menu.s.resume"]                             = "Resume";
	lang[? "menu.s.resume.desc"]                        = "Closes the Pause Menu and resumes gameplay.";
	lang[? "menu.s.title"]                              = "Quit to Title";

	lang[? "menu.e.hitboxes"]                           = "Display Hitboxes";
	lang[? "menu.e.damageBoxes"]                        = "Display Damage Boxes";
	lang[? "menu.e.paralax"]                            = "Paralax Amount";
	lang[? "menu.e.paralax.desc"]                       = "(Def: 1) The amount of apparent 3d movement of background and foreground assets.";
	lang[? "menu.e.showdebug"]                          = "Show Debug Overlay";
	lang[? "menu.e.showui"]                             = "Show UI";
	lang[? "menu.e.uiscale"]                            = "UI Scale";
	lang[? "menu.e.windowmode"]                         = "Window Mode";
	lang[? "menu.e.frameSkips"]                         = "Visual FPS";
	lang[? "menu.e.frameSkips.desc"]                    = "Increases preformance by reducing the amount of frames being drawn to the screen.";
	lang[? "menu.e."]                                   = "";

	lang[? "inventory.item.sword.name"]                 = "Broken Sword";
	lang[? "inventory.item.sword.desc"]                 = "It's a golden sword with a broken blade. If you focus energy into it, you can construct a new blade for it using essence.";

	lang[? "inventory.flower.none_selected"]            = "No Flower Selected";
	lang[? "inventory.flower.no_campfire"]              = "No Lit Campfire Nearby";
	lang[? "inventory.flower.effect"]                   = "Effect:";
	lang[? "inventory.flower.location"]                 = "Location:";
	lang[? "inventory.flower.persistence"]              = "Duration:";
	lang[? "inventory.flower.add_to_flame"]             = "Press ^continue to add to flame";
	lang[? "inventory.flower.cancel_selection"]         = "Press ^back to cancel";
	lang[? "inventory.flower.add_mutator"]              = "Press ^continue to add to flame";
	lang[? "inventory.flower.no_active_effects"]        = "You currently do not have any floral effects";
	lang[? "inventory.flower.effect_already_have"]      = "Flower Already in Flame";
	lang[? "inventory.flower.add_mutators"]             = "Add Mutator to Flower";

	lang[? "flower.0.name"]                             = "Lapis Radiance";
	lang[? "flower.1.name"]                             = "Scarlet Fawn";
	lang[? "flower.2.name"]                             = "Fuchsia Lily";
	lang[? "flower.3.name"]                             = "Amber Trillium";
	lang[? "flower.4.name"]                             = "Sun Daffodil";
	lang[? "flower.5.name"]                             = "Cloud Daisy";
	lang[? "flower.6.name"]                             = "Aquamarine Star";
	lang[? "flower.7.name"]                             = "Summer Flora";
	lang[? "flower.8.name"]                             = "Rose Sunflower";
	lang[? "flower.9.name"]                             = "Violet Lily";
	lang[? "flower.10.name"]                            = "NA";

	lang[? "flower.0.desc"]                             = "owowowowwowowowo";
	lang[? "flower.1.desc"]                             = "owowowowwowowowo";
	lang[? "flower.2.desc"]                             = "owowowowwowowowo";
	lang[? "flower.3.desc"]                             = "";
	lang[? "flower.4.desc"]                             = "fear";
	lang[? "flower.5.desc"]                             = "";
	lang[? "flower.6.desc"]                             = "";
	lang[? "flower.7.desc"]                             = "";
	lang[? "flower.8.desc"]                             = "";
	lang[? "flower.9.desc"]                             = "";
	lang[? "flower.10.desc"]                            = "";

	lang[? "flower.0.effect"]                           = "Health Boost !";
	lang[? "flower.1.effect"]                           = "Mana Gain !";
	lang[? "flower.2.effect"]                           = "?";
	lang[? "flower.3.effect"]                           = "?";
	lang[? "flower.4.effect"]                           = "?";
	lang[? "flower.5.effect"]                           = "?";
	lang[? "flower.6.effect"]                           = "?";
	lang[? "flower.7.effect"]                           = "?";
	lang[? "flower.8.effect"]                           = "?";
	lang[? "flower.9.effect"]                           = "?";
	lang[? "flower.10.effect"]                          = "?";

	lang[? "flower.0.location"]                         = "Various";
	lang[? "flower.1.location"]                         = "Firefly Forest";
	lang[? "flower.2.location"]                         = "?";
	lang[? "flower.3.location"]                         = "?";
	lang[? "flower.4.location"]                         = "?";
	lang[? "flower.5.location"]                         = "?";
	lang[? "flower.6.location"]                         = "?";
	lang[? "flower.7.location"]                         = "?";
	lang[? "flower.8.location"]                         = "?";
	lang[? "flower.9.location"]                         = "?";
	lang[? "flower.10.location"]                        = "?";

	lang[? "flower.mutator_name.0"]                     = "No Mutator";
	lang[? "flower.mutator_name.1"]                     = "Augmenting Heart";
	lang[? "flower.mutator_name.2"]                     = "Unyeilding Bone";


	lang[? "option.visible"]                            = "Visible";
	lang[? "option.hidden"]                             = "Hidden";
	lang[? "option.yes"]                                = "Yes";
	lang[? "option.no"]                                 = "No";
	lang[? "option.windowed"]                           = "Windowed";
	lang[? "option.fullscreen"]                         = "Fullscreen";
	lang[? "option.skip0"]                              = "No Frame Skips (60fps)";
	lang[? "option.skip1"]                              = "Skip Every Other Frame (30fps)";
	lang[? "option.skip2"]                              = "Draw every third frame (20fps)";
	lang[? "option.skip3"]                              = "Draw every fourth frame (15fps)";

	lang[? "test.don't.change"]                         = [["don\'t change appostraphgies in text", "sko\' f", ]];

	lang[? "flower.name.0"]                             = "Ruby Star Plant";

	lang[? "status.name.0"]                             = "";

	lang[? "beastiary.magpie"]                          = "Magpie";

	lang[? "player.name"]                               = "{te.name}Keira{te.fade}";

	lang[? "char.name.hugo"]                            = "{te.name}Hugo{te.fade}";
	lang[? "char.name.adaline"]                         = "{te.name}Adaline{te.fade}";

	lang[? "te.fade"]                                   = "#!0";
	lang[? "te.typewritter"]                            = "#!1";
	lang[? "te.wavy"]                                   = "#!2";
	lang[? "te.shake"]                                  = "#!3";
	lang[? "te.rainbow"]                                = "#!4";
	lang[? "te.order"]                                  = "#!5";
	lang[? "te.balanced"]                               = "#!6";
	lang[? "te.chaos"]                                  = "#!7";
	lang[? "te.name"]                                   = "#!8";
	lang[? "te.glyph"]                                  = "#!9";
	lang[? "te.reset"]                                  = "{te.fade}";

	lang[? "test"]                                      = [["Hey! Checking if the text works as intended. Let\'s see what {player.name} has to say.", "2", ], ["Wow, I can talk now.", "0", ], ["{te.rainbow}AAAAAAAAAAAA sunshines and rainbows!!!!!!!!!!!!!!!!!!!!!", "1", ], ["{te.shake}I will destroy the world!!", "1", ], ["I actually hate you.", "2", ], ["what.", "1", ]];
	lang[? "test2"]                                     = [["I\'m staying away from him.", "1", ], ["He creeps me out.", "1", ], ["Same.", "0", ], ["Let\'s get out of here.", "1", ]];
	lang[? "test3"]                                     = [["Hey! Come back!", "0", ]];
	lang[? "test4"]                                     = [["...", "0", ], ["I has cookies!", "0", ]];
	lang[? "test4.option1"]                             = "Take the bait";
	lang[? "test4.option2"]                             = "Ignore";

	lang[? "healstatue.inspect"]                        = [["This statue has writing underneath it.", "0", ], ["Somehow, like before, you are able to read it!", "0", ], ["\"Channel the earth, restore your marks through fire\".", "0", ], ["\"Restore\" is written prominently.", "0", ]];
	lang[? "healstatue.inspect.2"]                      = [["Maybe touching it will trigger something like before...", "0", ]];

	lang[? "instruction.essence_gather"]                = "Collect Essence by damaging infected objects or enemies.";
	lang[? "instruction.heal"]                          = "Hold ^heal to restore health by consuming essence. ";

	lang[? "empty"]                                     = "";

	lang[? "location.chasm"]                            = "Unsealed Chasm";
	lang[? "location.forest"]                           = "Firefly Forest";
}